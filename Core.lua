...
_Title.Size = UDim2.new(1, 0, 1, 0); _Title.Text = "🌚刘某某脚本🌝 V3.85"; _Title.Font = "GothamBold"; _Title.TextSize = 22; _Title.TextColor3 = Color3.new(1, 1, 1); _Title.BackgroundTransparency = 1
...
local _SBtn1 = _CreateSBtn("基本功能"); local _SBtn2 = _CreateSBtn("不用钥匙"); local _SBtn3 = _CreateSBtn("需要钥匙"); local _SBtn4 = _CreateSBtn("更新介绍")
...
local _CTitle = Instance.new("TextLabel", _ConfirmBox); _CTitle.Size = UDim2.new(1, 0, 0.45, 0); _CTitle.BackgroundTransparency = 1; _CTitle.Text = "加载脚本？"; _CTitle.TextColor3 = Color3.new(1,1,1); _CTitle.Font = "GothamBold"; _CTitle.TextSize = 18; _CTitle.ZIndex = 52
...
local function _AskToLoad(name, url) _PendingUrl = url; _CTitle.Text = "确认加载 ["..name.."] ?"; _ConfirmBlocker.Visible = true end
...
local function _CreateInput(txt, key, order, parent)
    ...
    i.FocusLost:Connect(function() _G_LMM_88[key] = math.clamp(tonumber(i.Text:match("%d+")) or 50, 0, 300); i.Text = txt..": ".._G_LMM_88[key] end)
end
...
_CreateT("安全穿墙", "v_0x2", 7, _Page1)
_CreateT("速度开关", "v_0x3", 8, _Page1)
_CreateT("飞行开关", "v_0x4", 9, _Page1)

-- 脚本库按钮 (示例)
local function _CreateScriptBtn(name, url, order, parent)
    local b = Instance.new("TextButton", parent); b.LayoutOrder = order; b.Size = UDim2.new(0.9, 0, 0, 55); b.BackgroundColor3 = Color3.fromRGB(35, 35, 35); b.Text = name; b.TextColor3 = Color3.new(1, 1, 1); Instance.new("UICorner", b)
...
-- [ 1. 穿墙逻辑优化 ]
-- 移至 Stepped 确保在物理引擎计算前执行，避免碰撞检测踢出
_RS.Stepped:Connect(function()
    if _G_LMM_88.v_0x2 and _LP.Character then
        for _, v in ipairs(_LP.Character:GetDescendants()) do
...
_RS.RenderStepped:Connect(function()
    ...
    if char and char:FindFirstChild("Humanoid") and char:FindFirstChild("HumanoidRootPart") then
        ...
        -- 速度优化：改用位移偏移，而不是暴力改 WalkSpeed 属性
        if _G_LMM_88.v_0x3 and hum.MoveDirection.Magnitude > 0 then
            hrp.CFrame = hrp.CFrame + (hum.MoveDirection * (_G_LMM_88.v_val_1 / 100))
        end
        
        -- 飞行优化：限制最大力，避免因加速度过快触发 Ban
        if _G_LMM_88.v_0x4 then
            if not _FlyVelo then 
                _FlyVelo = Instance.new("BodyVelocity", hrp)
                _FlyVelo.MaxForce = Vector3.new(4000, 4000, 4000) -- 合理的力值，不易拉回
            end
            _FlyVelo.Velocity = (hum.MoveDirection.Magnitude > 0 or _UIS:IsKeyDown(Enum.KeyCode.Space)) 
                and _Cam.CFrame.LookVector * _G_LMM_88.v_val_2 
                or Vector3.new(0, 0.1, 0) -- 微弱向上浮力防止掉落
