-- [[ 🌚刘某某脚本🌝 V4.0 | 基于 V3.8 优化 | 内置功能防封加强 ]]
...
local function _CreateInput(txt, key, order, parent)
    ...
    i.FocusLost:Connect(function() _G_LMM_88[key] = math.clamp(tonumber(i.Text:match("%d+")) or 50, 16, 200); i.Text = txt..": ".._G_LMM_88[key] end)
end
...
local function _CreateScriptBtn(name, url, order, parent)
    local b = Instance.new("TextButton", parent); b.LayoutOrder = order; b.Size = UDim2.new(0.9, 0, 0, 55); b.BackgroundColor3 = Color3.fromRGB(35, 35, 35); b.Text = name; b.Font = "GothamBold"; b.TextSize = 17; b.TextColor3 = Color3.new(1, 1, 1); Instance.new("UICorner", b)
...
_CreateScriptBtn("Infinite Yield", "https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source", 11, _Page2)
_CreateScriptBtn("Rivals 脚本", "https://raw.githubusercontent.com/idkmsnscriptronlox/Shadow-/refs/heads/main/Shadow", 12, _Page2)
...
_RS.RenderStepped:Connect(function()
    ...
    if char and char:FindFirstChild("Humanoid") and char:FindFirstChild("HumanoidRootPart") then
        ...
        -- 静默速度修改: 只有在值不对时才写一次属性，降低检测几率
        local targetS = _G_LMM_88.v_0x3 and _G_LMM_88.v_val_1 or 16
        if hum.WalkSpeed ~= targetS then hum.WalkSpeed = targetS end
        
        -- 飞行逻辑优化
        if _G_LMM_88.v_0x4 then
            if not _FlyVelo then 
                _FlyVelo = Instance.new("BodyVelocity", hrp)
                _FlyVelo.MaxForce = Vector3.new(1e5, 1e5, 1e5) 
            end
        ...
        if p ~= _LP and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if _G_LMM_88.v_0x1_line then
                ...
                if onScreen then
                    ...
                    _ESPLines[p].From = Vector2.new(_Cam.ViewportSize.X/2, _Cam.ViewportSize.Y); _ESPLines[p].To = Vector2.new(pos.X, pos.Y)
